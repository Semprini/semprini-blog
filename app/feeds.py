# Copy of puput feeds but fixed item_enclosure_length to check for local file.
# If not using the basic filesystem (e.g. S3) for media then length will not work
import os
...
class BlogPageFeed(Feed):
    ...
    def _channel_image_link(self):
        if self.blog_page.header_image:
            site = Site.find_for_request(self.request)
            # base_url = getattr(settings, "MEDIA_URL", site.root_url)
            # return urljoin(base_url, self.blog_page.header_image.file.url)
